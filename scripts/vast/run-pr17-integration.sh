...
# The guest is NOT built here. guest/dist/app is committed to the repo (built
# reproducibly and refreshed via /update-vks), and the committed FRI VK embeds
# sha256(app.bin), so the cloned checkout already carries a matching guest +
# VKs. Verify it's present rather than rebuilding.
log_step "Verify committed guest dist"
if [ ! -f "$repo_dir/guest/dist/app/app.bin" ]; then
  echo "error: $repo_dir/guest/dist/app/app.bin missing from the checkout." >&2
  echo "The guest dist is committed; ensure REPO_REF includes it." >&2
  exit 1
fi
run ls -lh "$repo_dir/guest/dist/app"
