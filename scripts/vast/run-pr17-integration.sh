...
# Build the guest reproducibly. `--reproducible` compiles inside a pinned
# Docker container with a fixed mount path, so app.bin (and the FRI VK derived
# from it) is bit-for-bit identical to the committed key generated by CI. This
# requires a usable Docker daemon on the rented instance — launch the Vast
# template with Docker/DinD enabled, otherwise this step fails fast below.
log_step "Build Airbender guest (reproducible)"
if ! docker info >/dev/null 2>&1; then
  echo "error: 'docker info' failed — the reproducible guest build needs a Docker daemon." >&2
  echo "Launch the Vast instance with Docker enabled (privileged/DinD)." >&2
  exit 1
fi
run cargo airbender build --reproducible --workspace-root . --project guest
