...
/// Cheap compatibility check: load the batch, unwrap the V1 payload, return its
/// protocol version. No guest, no VM execution.
...
fn process_batch(app_bin_dir: &Path, bf: &BatchInputFile) -> Result<DatasetRow> {
    ...
    let guest = run_guest(app_bin_dir, &envelope)
